...
void opcontrol() {
	...
	while (true) {
		...
		//vision sensor start
		pros::vision_object cube = vision_sensor.get_by_sig(0, 1); // yesil dude
		...
		{
				pros::vision_object cube = vision_sensor.get_by_sig(0,1);
				power = 0;
				turn = 0;
			...
			{
				turn = (cube.x_middle_coord-155) / 7;
				pros::lcd::print(6, "imma be turnin'"); //3.5 for competition
			}
			else pros::lcd::print(6,"aint turnin for now");
			if((cube.height >= 30 || cube.width >=30) && (cube.height <= 200 || cube.width <=200) && cube.x_middle_coord > 0) power = 40; // if there is a cube(cube.x_middle_coord > 0) and if it is in the appropriate size, RUN INTO IT!!!!
